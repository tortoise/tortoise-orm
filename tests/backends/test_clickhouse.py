"""
Test some clickhosue-specific features
"""
import ssl
...
class TestClickHouse(test.SimpleTestCase):
    async def setUp(self):
        if Tortoise._inited:
            await self._tearDownDB()
        self.db_config = test.getDBConfig(app_label="models", modules=["tests.testmodels"])
        if self.db_config["connections"]["models"]["engine"] != "tortoise.backends.asynch":
            raise test.SkipTest("ClickHouse only")
