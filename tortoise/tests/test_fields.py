...
from tortoise.exceptions import (ConfigurationError, IntegrityError, NoValuesFetched,
                                 OperationalError)
...
class TestForeignKeyField(test.TestCase):
    ...
    async def test_event__order_by(self):
        ...
        event1 = await testmodels.Event.create(name='Event1', tournament=tour)
        event2 = await testmodels.Event.create(name='Event2', tournament=tour)
        self.assertEqual(await tour.events.all().order_by('-name'), [event2, event1])
        self.assertEqual(await tour.events.all().order_by('name'), [event1, event2])
    ...
    async def test_event__limit(self):
        ...
        self.assertEqual(await tour.events.all().order_by('name').limit(2), [event1, event2])
    ...
    async def test_event__offset(self):
        ...
        self.assertEqual(await tour.events.all().order_by('name').offset(1), [event2, event3])
